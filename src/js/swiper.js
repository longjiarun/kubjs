!(function(root,factory){
    ...
    function init(){
        ...
        hammer.on("panstart",function(event){   
            ...
            if(options.followFinger){
                if(x != void 0){
                    //横向
                    var resistance = 1 - event.distance/w/2;
                    if(self.activeIndex == 0){
                        resistance = event.deltaX > 0 ? resistance : 1;
                    }else if( self.activeIndex == self.length-1 ){
                        resistance = event.deltaX < 0 ? resistance : 1 ;
                    }else{
                        resistance=1;
                    }
                    self.setTranslate(x + event.deltaX * resistance + "px", 0,0);
                }else{
                    //垂直
                    var resistance = 1 - event.distance/h/2;
                    resistance = resistance < 0.5 ? 0.5 : resistance;
                    if(self.activeIndex == 0){
                        resistance = event.deltaY > 0 ? resistance : 1;
                    }else if( self.activeIndex == self.length-1 ){
                        resistance = event.deltaY < 0 ? resistance : 1 ;
                    }else{
                        resistance=1;
                    }
                    self.setTranslate(0,y + event.deltaY * resistance + "px",0);
                }
            }
            event.preventDefault();
    ...
    ;(function(){
        ...
        this.setTranslate = function(x,y,speed,keep){
        ...
        this.getDuration = function(distance,velocity){
            ...
            return duration > this.options.speed ? this.options.speed : duration;
        };
        ...
        this.slideTo = function(index,speed){
            ...
            if(options.direction === "horizontal"){
                ...
                self.setTranslate( -self.activeIndex * w + "px",0,speed).setTranslate( -index * w + "px",0,speed);
            }else{
                //垂直
                var h = self.ui.slides.eq(0).height();
                self.setTranslate(0,-self.activeIndex * h  + "px",speed).setTranslate(0,-index * h  + "px",speed);
            }
