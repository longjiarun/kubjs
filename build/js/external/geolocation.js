...
!(function(root,factory){
    if(typeof define === "function"){
        ...
        root.geolocation = factory(root,root.jQuery || root.Zepto);
    }
}(this,function(root,$){
    /**
     ...
     * 通过经纬度获取地址接口，用的是百度的API，线上环境也可以使用，但需要申请key。最好自己提供这样的接口
     */
    var geolocation={
        options:{
            ipUrl:"http://int.dpool.sina.com.cn/iplookup/iplookup.php?format=js", //通过ip获取位置的url
            //采用百度 API 需要申请key,下面的key在本地可以进行测试
            //http://developer.baidu.com/map/index.php?title=webapi/guide/webservice-geocoding
            positionUrl:"http://api.map.baidu.com/geocoder/v2/?ak=8827ee39511568ac0705d037d67b2624&callback=renderReverse&output=json&pois=1",//通过经纬度获取位置的url
            expires:172800000, // 两天 单位：毫秒。位置cookie 存放时间
            cookieName:"geo",
            enableHighAcuracy: false,// 指示浏览器获取高精度的位置，默认为false
            timeout: 4000,//单位为毫秒 指定获取地理位置的超时时间
            //单位为毫秒  指定 geo 等待超时时间，由于在获取经纬度时会得到用户的许可，这期间可能时间较长，页面可能由于获取不到数据
            geoWaitTime :5000, 
            maximumAge: 172800000 //两天 单位为毫秒 最长有效期，在重复获取地理位置时，此参数指定多久再次获取位置。
        },
        log:function(){
        ...
        getAddress : function(callback,error){
            var self=this,options = self.options,city,flag,address;
                ...
                options.positionUrl && $.ajax({
                    ...
                    success:function(data){
                        if(data && data.result && data.result.addressComponent){
                            self.storeAddress((address = self.format(data.result.addressComponent)));
